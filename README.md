...
- ⚠️ 2D/3D area calculations are no longer available; only GeoJSON outputs are produced
...
> **Note:** The `--workers` flag uses multiprocessing and doesn't work on Windows. For production deployments on Windows, consider using Docker or deploying to a Linux server.
...
- The 2D and 3D area calculations have been removed from the backend. Only GeoJSON outputs for 2D and 3D ruimtebeslag are produced.
- If you run test scripts in the dev/ folder (e.g., test_api_output_to_files.py), you must set the PYTHONPATH to the project root so that imports like from app.volume_calc import DikeModel work correctly.

**Example (Windows PowerShell):**

```powershell
$env:PYTHONPATH='.'; python dev/test_api_output_to_files.py
```

If you are in the dev folder, run:
...
│   ├── __init__.py
│   ├── volume_calc.py      # Dike volume calculation logic (no area calcs)
│   ├── AHN_raster_API.py   # AHN4 raster API wrapper
│   └── AHN5.py             # AHN5 API implementation
├── dev/                    # Development and testing files
│   ├── README.md
│   ├── test_*.py           # Test files (set PYTHONPATH when running)
│   ├── sandbox.py          # Experimental code
│   └── fix_dataclass.py    # Development utilities
...
| `/api/calculate_designs` | POST | Yes | Calculate dike volume and return points for ruimtebeslag (no GeoJSON) |
| `/api/debug_calculate_volume` | POST | Yes | Debug volume calculation |


## Example Response for `/api/calculate_designs`
...
```json
{
  ...
  "calculation_time": 0.456,
  "ruimtebeslag_2d_points": [[x, y], [x, y], ...] // EPSG:3857
}
```

**Note:** The backend now returns only a list of points (`ruimtebeslag_2d_points`, in EPSG:3857) for ruimtebeslag calculation in the frontend. The 2D ruimtebeslag GeoJSON is no longer returned.
