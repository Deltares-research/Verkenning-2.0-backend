...
class VerankerdeDamwandModel(StructureModel):
    """Model representing a verankerde damwand structure."""

    def set_cost_function_parameters(self) -> float:
        """Compute the total cost of the onverankerde damwand based on its properties."""
        self.cost_function_parameters = {'c': get_price(self.cost_catalog, 'c_Verankerd'),
                                'd': get_price(self.cost_catalog, 'd_Verankerd'),
                                'z': get_price(self.cost_catalog, 'z_Verankerd')}