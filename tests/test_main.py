...
def test_cost_calculation_for_ground_design_envelope(gdf_ground_base):
    ...
    np.testing.assert_allclose(response.json()["breakdown"]['Bouwkosten']['Indirecte Bouwkosten']['totale_bouwkosten']['value_excl_BTW'], 95104.352023, rtol=1e-2)
    np.testing.assert_allclose(response.json()["breakdown"]['Risicoreservering']['value'], 18749.93, rtol=1e-2)
...
def test_cost_calculation_for_ground_and_structure(gdf_ground_base, gdf_structure):
    ...
    response = client.post("/api/cost_calculation", json=payload, headers={"X-API-Key": os.getenv("API_KEY")})
    assert response.status_code == 200
    np.testing.assert_allclose(response.json()["breakdown"]['Bouwkosten']['Indirecte Bouwkosten']['totale_BDBK_grondwerk']['value_excl_BTW'], 68294.29, rtol=1e-2)
