...
def sum_values_from_cost_dict(cost_dict):
    """Helper function to sum all 'value' entries in a cost dictionary. Only uses cost components, not those excl/incl BTW. So SUmmedCostValues are ignored"""
...
def test_dike_model_cost_computation_with_ground_only(gdf_ground):
    ...
    np.testing.assert_allclose(sum(values), 87977.00, rtol=1e-2)
    np.testing.assert_allclose(cost_dict['Bouwkosten']['Directe Bouwkosten']['Directe kosten grondwerk']['totale_BDBK_grondwerk']['value_excl_BTW'], 87977.00, rtol=1e-2)
    np.testing.assert_allclose(cost_dict['Bouwkosten']['Directe Bouwkosten']['Directe kosten grondwerk']['totale_BDBK_grondwerk']['value_incl_BTW'], 87977.00*1.21, rtol=1e-2)
...
def test_dike_model_cost_computation_with_both(gdf_structure, gdf_ground):
    ...
    np.testing.assert_allclose(cost_dict['Bouwkosten']['Directe Bouwkosten']['Directe kosten grondwerk']['totale_BDBK_grondwerk']['value_excl_BTW'], 87977.00, rtol=1e-2)
    np.testing.assert_allclose(sum_values_from_cost_dict(cost_dict['Bouwkosten']['Directe Bouwkosten']['Directe kosten constructies']), 417198.65, rtol=1e-2)
    np.testing.assert_allclose(sum_values_from_cost_dict(cost_dict['Bouwkosten']['Indirecte Bouwkosten']), 207656.54, rtol=1e-2)
...
def test_dike_model_cost_computation_with_heavescreen(gdf_structure):
    ...
    np.testing.assert_allclose(float(sum_values_from_cost_dict(cost_dict['Bouwkosten']['Directe Bouwkosten']['Directe kosten constructies'])), 373981.51)  #example value
...
def test_dike_model_cost_computation_with_verankerde_damwand_is_more_expensive_than_onverankerde(gdf_structure):
    ...
    gdf_structure_verankerde.loc[0,'type'] = 'Verankerde damwand'

    dike_model_onverankerde = DikeModel(_2d_structure = gdf_structure, complexity='makkelijke maatregel')
    cost_dict_onverankerde = dike_model_onverankerde.compute_cost(nb_houses=0, road_area=0)
...
def test_opslagfactoren_for_structures_should_be_identical(gdf_structure):
    ...
    for key in cost_keys:
        np.testing.assert_allclose(
            cost_dict_onverankerde['Engineeringkosten'][key]['value'] / cost_dict_onverankerde['Bouwkosten']['Directe Bouwkosten']['Directe kosten constructies']['totale_BDBK_constructie']['value_excl_BTW'],
            cost_dict_verankerde['Engineeringkosten'][key]['value'] / cost_dict_verankerde['Bouwkosten']['Directe Bouwkosten']['Directe kosten constructies']['totale_BDBK_constructie']['value_excl_BTW'],
            rtol=1e-2
        )
