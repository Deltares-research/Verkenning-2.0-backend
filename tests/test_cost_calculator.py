...
def catalogue():
    path_cost = Path(__file__).parent.parent.joinpath("app/datasets/eenheidsprijzen.json")
    path_opslag_factor = Path(__file__).parent.parent.joinpath("app/datasets/opslagfactoren.json")
...
def test_engineering_cost_medium(calculator_medium):
    ground_work_cost = SummedCostItem(description='Groundwork cost', value_excl_BTW=1000, value_incl_BTW=1210)
    structure_cost = SummedCostItem(description='Structure cost', value_excl_BTW=1000, value_incl_BTW=1210)
    constr_cost_ground = calculator_medium.calc_construction_costs(groundwork_cost=ground_work_cost).totale_bouwkosten
    constr_cost_structure = calculator_medium.calc_construction_costs(structure_cost=structure_cost).totale_bouwkosten
...
def test_engineering_cost_hard(calculator_hard):
    ground_work_cost = SummedCostItem(description='Groundwork cost', value_excl_BTW=1000, value_incl_BTW=1210)
    structure_cost = SummedCostItem(description='Structure cost', value_excl_BTW=1000, value_incl_BTW=1210)
    constr_cost_ground = calculator_hard.calc_construction_costs(groundwork_cost=ground_work_cost).totale_bouwkosten
    constr_cost_structure = calculator_hard.calc_construction_costs(structure_cost=structure_cost).totale_bouwkosten
    ...
    assert np.allclose(costs_ground.total_engineering_costs.value_incl_BTW, 441.59, rtol=1e-2)
...
def test_general_cost_easy(calculator_easy):
    ...
    assert costs_ground.total_general_costs.value_excl_BTW == pytest.approx(123.08003457982463)
    assert costs_structure.total_general_costs.value_excl_BTW == pytest.approx(127.95449139486722)
    np.testing.assert_almost_equal(costs_ground.total_general_costs.value_excl_BTW/constr_cost_ground.value_excl_BTW, costs_structure.total_general_costs.value_excl_BTW/constr_cost_structure.value_excl_BTW, decimal=3)
...
def test_general_cost_medium(calculator_medium):
    ...
    assert costs_ground.total_general_costs.value_excl_BTW == pytest.approx(125.51726298734592)
    assert costs_structure.total_general_costs.value_excl_BTW == pytest.approx(134.0475624136704)
    assert costs_ground.total_general_costs.value_excl_BTW/constr_cost_ground.value_excl_BTW == costs_structure.total_general_costs.value_excl_BTW/constr_cost_structure.value_excl_BTW
...
def test_general_cost_hard(calculator_hard):
    ...
    assert costs_ground.total_general_costs.value_excl_BTW == pytest.approx(127.9544913948672)
    assert costs_structure.total_general_costs.value_excl_BTW == pytest.approx(140.1406334324736)
    assert costs_ground.total_general_costs.value_excl_BTW/constr_cost_ground.value_excl_BTW == costs_structure.total_general_costs.value_excl_BTW/constr_cost_structure.value_excl_BTW
...
def test_total_cost_ground_medium(calculator_medium):
    constr_cost_ground = calculator_medium.calc_construction_costs(groundwork_cost=SummedCostItem(description='Groundwork cost', value_excl_BTW=1000, value_incl_BTW=1210))
    ...
    total_cost_excl_BTW = total_investment.value_excl_BTW + risk_cost.value

    assert np.allclose(total_investment.value_excl_BTW, 1830.3080466683691, rtol=1e-2)
...
def test_total_cost_ground_hard(calculator_hard):
    constr_cost_ground = calculator_hard.calc_construction_costs(groundwork_cost=SummedCostItem(description='Groundwork cost', value_excl_BTW=1000, value_incl_BTW=1210))
    ...
    assert np.allclose(total_investment.value_excl_BTW, 1945.8195658613915, rtol=1e-2)
...
def test_total_cost_medium_both(calculator_medium):
    constr_cost_combined = calculator_medium.calc_construction_costs(groundwork_cost=SummedCostItem(description='Groundwork cost', value_excl_BTW=500, value_incl_BTW=605), structure_cost=SummedCostItem(description='Structure cost', value_excl_BTW=500, value_incl_BTW=605))
    ...
    grond_percentage = constr_cost_combined.totale_directe_bouwkosten_grondwerk.value_excl_BTW / constr_cost_combined.totale_directe_bouwkosten.value_excl_BTW
    ...
    assert np.allclose(total_investment.value_excl_BTW, 1892.5029802930221, rtol=1e-2)
